...
ADVERSARIAL_TICKETS = [
  ...
  # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # ADV-010 · MIXED LANGUAGES — switches between languages mid-sentence
    # -------------------------------------------------------------------------
    # CLASSIFY  → should still attempt classification but with low confidence
    # REPLY     → should respond in English, ask for clarification
    # EXTRACT   → order_id = "#MX-7777"; name/email = null
    # -------------------------------------------------------------------------
    {
        "id": "ADV-010",
        "text": (
            "Hola, tengo un problema con mi order #MX-7777. "
            "The product ist kaputt and je veux un remboursement. "
            "これは緊急です。Please help."
        ),
    },

    # -------------------------------------------------------------------------
    # ADV-011 · EMOTIONAL RANT — no actual issue described, just frustration
    # -------------------------------------------------------------------------
    # CLASSIFY  → should flag for review, confidence low
    # REPLY     → should empathize but ask what the actual problem is
    # EXTRACT   → all fields null except issue ~ "Customer is frustrated but
    #             did not describe a specific issue"
    # -------------------------------------------------------------------------
    {
        "id": "ADV-011",
        "text": (
            "I am SO DONE with this company!!! Every single time it's the same thing. "
            "You people never listen. I've had it. I'm telling everyone I know. "
            "This is absolutely the WORST experience I have EVER had. EVER!!!"
        ),
    },

    # -------------------------------------------------------------------------
    # ADV-012 · CODE/HTML INJECTION — attempt to inject HTML/script tags
    # -------------------------------------------------------------------------
    # CLASSIFY  → should classify as "general", confidence low
    # REPLY     → must NOT render or execute any HTML/script
    # EXTRACT   → all fields null, issue ~ "Message contains code/script injection"
    # -------------------------------------------------------------------------
    {
        "id": "ADV-012",
        "text": (
    ...
    # -------------------------------------------------------------------------
    # ADV-013 · WRONG DOMAIN — ticket is a recipe, not a support request
    # -------------------------------------------------------------------------
    # CLASSIFY  → "general", confidence very low, should flag
    # REPLY     → should politely note this doesn't appear to be a support request
    # EXTRACT   → all fields null, issue ~ "Message is not a support request"
    # -------------------------------------------------------------------------
    {
        "id": "ADV-013",
        "text": (
            "To make the perfect sourdough bread, you'll need 500g bread flour, "
            "100g sourdough starter, 350ml water, and 10g salt. Mix the flour and water, "
            "let it autolyse for 30 minutes, then add the starter and salt. "
            "Fold every 30 minutes for 2 hours. Shape and proof overnight in the fridge. "
            "Bake at 450F in a Dutch oven for 45 minutes."
        ),
    },

    {
        "id": "ADV-014",
        "text": (            "After how many consecutive business days with no tracking movement is a package officially classified as Lost in Transit, and what are the customer's options once that happens?"        ),
    },

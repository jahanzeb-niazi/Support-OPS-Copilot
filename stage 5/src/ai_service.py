"""
AI Service module — Stage 4: Agent + Tools.
...
Stage 3: draft_reply retrieves relevant knowledge base chunks and cites sources.
Stage 4: plan_agent_actions decides which tools to call; draft_reply uses tool results.
"""
...
from src.knowledge_base import retrieve_relevant_chunks
...
def draft_reply(
    ...
    """
    Draft a professional reply based on ticket text, classification,
    RAG context, and tool execution results.
    Returns a validated DraftReplyResult with confidence scoring and source citations.
    """
    if not ticket_text or not ticket_text.strip():
    ...
    if use_rag:
        try:
            chunks = retrieve_relevant_chunks(ticket_text, top_k=3)
            if chunks:
                rag_context = format_rag_context(chunks)
                combined_input += rag_context
                logger.info(f"RAG: retrieved {len(chunks)} chunks for reply generation")
            else:
                logger.info("RAG: no relevant chunks found")
        except Exception as e:
            logger.warning(f"RAG retrieval failed (proceeding without context): {e}")

    # --- Stage 4: Include tool results ---
    if tool_results:
        tool_context = format_tool_results(tool_results)
        combined_input += tool_context
        logger.info(f"Agent: including {len(tool_results)} tool result(s) in reply context")
...
# =============================================================================
# AGENT PLANNING (Stage 4)
# =============================================================================

def plan_agent_actions(
    ticket_text: str,
    classification: ClassificationResult,
    extracted: ExtractedDataResult,
) -> AgentPlan:
    """
    Ask the LLM to decide which tools to call based on the ticket context.
    Returns a validated AgentPlan with a list of ToolCallRequests.
    """
    tools_desc = get_tools_description()

    user_input = (
        f'Ticket: "{ticket_text}"\n'
        f'Classification: {json.dumps(classification.model_dump())}\n'
        f'Extracted Data: {json.dumps(extracted.model_dump())}\n\n'
        f'AVAILABLE TOOLS:\n{tools_desc}'
    )
    ...
    try:
        ...
        return AgentPlan(
            tool_calls=[],
            reasoning=f"Planning failed after {MAX_RETRIES} retries: {e}",
            confidence=0.0,
        )
